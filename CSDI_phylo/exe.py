...
def sort_species(args, data, subjectID, timepoint, train_index, foldername):
    ...
    attributes = list(final_data_df['otu'])
...
def main(args, config):
    ...
    # Read data
    data, subjectID, timepoint, taxons = read_data(args.dataset_path)
    ...
    dataset = diabimmune_Dataset(
        ...
        missing_ratio=args.testmissingratio, seed=args.seed, attributes=None
    )
    ...
    for _fold, (train_index, test_index) in enumerate(kf.split(dataset)):
        ...
        attributes = sort_species(args, data, subjectID, timepoint, train_index, foldername)
        ...
        # Training & evaluation
        cluster_size = get_cluster_size(attributes)
        model = CSDI_diabimmune(config, args.device, len(taxons), cluster_size).to(args.device)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset_path", type=str, default="../indata/DIABIMMUNE_16S/rel-species-table_clr.csv")
