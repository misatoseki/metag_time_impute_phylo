...
class Tokenizer(nn.Module):
    ...
    def forward(self, x_num: Tensor, x_cat: ty.Optional[Tensor]) -> Tensor:
        ...
        x = torch.cat([x_num, x_cat_embed], dim=2) # [B, time, num_taxon + num_cat*d_token]
...
class CSDI_diabimmune(CSDI_base):
    ...
    def process_data(self, batch):
        ...
        observed_data = self.tokenizer(
            observed_data[:, :, :(observed_data.shape[2] - self.num_cat)], # [B, time, taxon]
            observed_data[:, :, -self.num_cat:], # [B, time, num_cat]
        ) # [B, time, (taxon + num_cat * d_token)]
        ...
        ones_to_add = torch.ones(gt_mask.shape[0], gt_mask.shape[1], 
                                 observed_data.shape[2] - gt_mask.shape[2], dtype=gt_mask.dtype)
        gt_mask = torch.cat([gt_mask, ones_to_add], dim=2).to(self.device).float()
