    ...
    for _fold, (train_index, test_index) in enumerate(kf.split(x_all)):
        ...
        for i in range(len(x_mask_trans)):
            for j in range(len(x_mask_trans[i])):
                mean_val = pd.Series(x_mask_trans[i][j]).mean()
                impute_vector[i][j] = pd.Series(x_mask_trans[i][j]).fillna(mean_val)
        ...
        impute_df.to_csv("../outdata/imputed_Mean_clr_MR-" + str(missing_ratio) + "_fold" + str(_fold) + ".csv")
