...
dataset = diabimmune_Dataset(eval_length=7, missing_ratio=args.testmissingratio, seed=args.seed, attributes=attributes)

mae_cv = []
kf = KFold(n_splits=5, shuffle=True, random_state=1)
for _fold, (train_index, test_index) in enumerate(kf.split(dataset)):
        foldername = "./save/bonus_" + current_time + "_MR-" + str(args.testmissingratio) + "_fold" + str(_fold) + "/"
        ...
        model = CSDI_diabimmune(config, args.device, len(attributes), cluster_size).to(args.device)
