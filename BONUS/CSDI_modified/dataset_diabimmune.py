...
def process_func(path: str, missing_ratio=0.1, attributes=None, eval_length=6, mask=None):
    ...
    obs_mask = pd.read_csv("../indata/mask.csv", index_col=0)
    obs_mask = obs_mask.loc[np.sort(obs_mask.index)]
    obs_mask_array = np.array(obs_mask)[:, :, np.newaxis]
    observed_masks = np.tile(obs_mask_array, observed_values.shape[2])

    mask_array = np.array(mask)[:, :, np.newaxis]
    gt_masks = np.tile(mask_array, observed_values.shape[2])
...
class diabimmune_Dataset(Dataset):
    def __init__(self, eval_length=6, use_index_list=None, missing_ratio=0.0, seed=0, attributes=None, inputdir=""):
        ...
        dataset_path = "../indata/profile_Species_clr.csv"
...
def get_dataloader(seed=1, nfold=None, batch_size=16, missing_ratio=0.1, inputdir="", attributes=None, foldername="",
    ...
    dataset = diabimmune_Dataset(
        eval_length=7, use_index_list=train_index, missing_ratio=missing_ratio, seed=seed, attributes=attributes
    )
    train_loader = DataLoader(dataset, batch_size=batch_size, shuffle=1)
    valid_dataset = diabimmune_Dataset(
        eval_length=7, use_index_list=valid_index, missing_ratio=missing_ratio, seed=seed
    )
    valid_loader = DataLoader(valid_dataset, batch_size=batch_size, shuffle=0)
    test_dataset = diabimmune_Dataset(
        eval_length=7, use_index_list=test_index, missing_ratio=missing_ratio, seed=seed
    )
